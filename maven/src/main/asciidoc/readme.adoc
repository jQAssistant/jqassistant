...
=== Multiple projects per reactor

The jQAssistant Maven plugin uses a root Maven module where the following items are expected or created:
...
The use of the https://maven.apache.org/extensions/maven-build-cache-extension[Maven Build Cache Extension] (e.g. for large reactors) in combination with the jQAssistant Maven plugin requires the following setup in the cache configuration:

.mvn/maven-build-cache-config.xml
[source,xml]
----
<cache
	xmlns="http://maven.apache.org/BUILD-CACHE-CONFIG/1.2.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	...
	<configuration>
		<attachedOutputs>
            <!-- jQAssistant scans the classes directories which are not cached by default -->
			<dirNames>
	...
	<executionControl>
		<runAlways>
			<goalsLists>
                <!-- Ensures that the jQAssistant plugin is always executed -->
				<goalsList groupId="com.buschmais.jqassistant" artifactId="jqassistant-maven-plugin">
					<goals>
						<goal>scan</goal>
						<goal>analyze</goal>
					</goals>
