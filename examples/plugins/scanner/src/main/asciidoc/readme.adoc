...
A CSV file is represented by _CSVFileDescriptor_ which inherits from _CSVDescriptor_ and _FileDescriptor_ which is
defined by the scanner API. Thus a node of this type will carry the labels "CSV" and "File". Furthermore a list of
"HAS_ROW" relations is defined by the property "rows".
