...
- by deploying a rule extension, this is demonstrated by the example "ruleextension"

Now the following settings can be applied to the jQAssistant Maven plugin in the pom.xml of the project to be analyzed:

[source,xml]
----
include::{docRoot}/jqassistant.examples.sonar.project/src/pom.xml[lines=30..58]
----
