...
[source]
----
jqassistant exec -q "MATCH (n) RETURN count(n) as count"
jqassistant exec -qf query.cypher
jqassistant exec -q "CALL apoc.export.cypher.all(null, {stream: true})" -Djqassistant.store.embedded.apoc-enabled=true
----
...
* -qf, --query-file <file>
** Path to a file containing the Cypher query to execute
