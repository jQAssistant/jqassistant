...
* Added message during analysis for concepts which are skipped because they have already been applied in a previous execution.
* Fixed an issue where diagram reports (e.g. GraphML) failed with an exception (`"Element type not supported:"`)
* Upgrade to PlantUML 1.2022.2 and JRuby 9.2.20.1
...
+
NOTE: Plugins must declare `com.buschmais.jqassistant.core:test`as dependency (test-scoped).
