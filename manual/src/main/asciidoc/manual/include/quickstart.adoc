...
- the following JVM parameters are required if `Java 17` or later is used as runtime environment:
+
.Windows
----
...
* The latest command line distribution is available on https://search.maven.org/search?q=a:jqassistant-commandline-distribution[Maven Central], select `bin.zip` from the `Download` menu {projectVersion}.
