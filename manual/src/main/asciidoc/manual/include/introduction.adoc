...
Furthermore, the setting `jqassistant.analyze.report.break-on-failure` (default: `true`) controls whether jQAssistant will break the buildif at least one failure has been detected during analysis.

TIP: If  violations are exported to dashboards (e.g. SonarQube, Teamscale) for managing them there then `jqassistant.analyze.report.break-on-failure` should be set to `false`.
