...
In the first analysis the existing violations will be reported.
At the same time a file `jqassistant-baseline.xml` will be created in the rule directory containing these violations as entries.
These will no longer be reported by subsequent executions.
