...
NOTE: The rule XML schema is available https://jqassistant.github.io/jqassistant/current/schema/jqassistant-rule-v2.9.xsd[online] and should be picked automatically for code completion by your IDE.
...
Concepts can be marked as `abstract`. Abstract concepts ensure that relevant preconditions have already been taken into account by related
concepts (concepts providing an abstract concept). In the future, abstract concepts will only be allowed to perform read-only queries.
...
<jqassistant-rules xmlns="http://schema.jqassistant.org/rule/v2.9"
    ...
    <concept id="plugin:AbstractConcept" abstract="true">
        <cypher>
          ...
        </cypher>
    </group>
...
<constraint id="xmlExample:JavaScriptConstraint">
    ...
    <script language="JavaScript">
        ...
        verificationResult = context.verify(rule, columnNames, rows);
        com.buschmais.jqassistant.core.report.api.model.Result.builder()
          .rule(rule)
          .verificationResult(verificationResult)
          .status(status)
          .severity(severity)
          .columnNames(columnNames)
          .primaryColumn(java.util.Optional.of("Type"))
          .rows(rows)
          .build();
    </script>
</constraint>
...
By default the values of all columns returned by a constraint or a concept are used
to calculate a key for each row. These keys are used to uniquely identify rows between different executions,
for baselines or reporting to other tools, e.g. SonarQube or Teamscale.
In case the result contains additional columns with context information,
the columns to be used for calculating the row key can be specified separately:
...
<concept id="my-rules:MyConcept">
    ...
    <report keyColumns="m, n" />
</concept>
----

===== Overriding Rules

Groups, concepts, and constraints can override rules of the same type.
This results in the overriding rule being executed instead of the rule that was overridden.
However, a rule may only be overridden by one rule.

[source,xml]
----
<group id="group:OverridingGroup">
    <overridesGroup refId="group:OverriddenGroup"/>
</group>
----

[source,xml]
----
<concept id="concept:OverridingConcept">
    <overridesConcept refId="concept:OverriddenConcept" />
    <cypher><![CDATA[
        MATCH
          (m)-[]->(n)
          ...
...
----
NOTE: Overriding concepts must provide the same concepts as the concepts being overridden.

[source,xml]
----
<constraint id="constraint:OverriddenConstraint">
    <overridesConstraint refId="constraint:OverriddenConstraint" />
    <cypher><![CDATA[
        MATCH
          (m)-[]->(n)
          ...
