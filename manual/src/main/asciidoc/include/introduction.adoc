...
to calculate a key for each row which are used to uniquely identify rows between different executions.
These keys are used for baselines or reporting to other tools, e.g. SonarQube or Teamscale.
