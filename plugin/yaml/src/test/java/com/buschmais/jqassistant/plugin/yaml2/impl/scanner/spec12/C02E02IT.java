...
class C02E02IT extends AbstractYAMLPluginIT {
    private static String YAML_FILE = "/spec-examples/c2-e02-mapping-scalars-to-scalars.yaml";
    ...
    void theKeysHaveTheCorrectValuesAssigned() {
        ...
        YMLMapDescriptor map = getDocuments(ymlFileDescriptor).getDocumentByParsePosition(0)
                                                .getMaps().get(0);

        YMLSimpleKeyDescriptor keyHr = getKeys(map).getKeyByName("hr");
        YMLSimpleKeyDescriptor keyAvg = getKeys(map).getKeyByName("avg");
        YMLSimpleKeyDescriptor keyRbi = getKeys(map).getKeyByName("rbi");

        assertThat(keyHr).hasScalarValue("65");
        assertThat(keyAvg).hasScalarValue("0.278");
        assertThat(keyRbi).hasScalarValue("147");
    }
    ...
    void theDocumentContainsAMap() {
        YMLFileDescriptor yamlFileDescriptor = readSourceDocument();
        YMLDocumentDescriptor documentDescriptor = getDocuments(yamlFileDescriptor).getDocumentByParsePosition(0);
