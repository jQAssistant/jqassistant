...
class ByteCodeVersionIT extends AbstractJavaPluginIT {
    ...
    void byteCodeVersion() {
        ...
        List<TypeClassFileDescriptor> types = query("MATCH (t:Type) WHERE t.name='Pojo' RETURN t").getColumn("t");
        assertThat(types).hasSize(1);
        TypeClassFileDescriptor pojo = types.get(0);
    ...
    void javaVersion() throws Exception {
        ...
        store.beginTransaction();
        List<TypeClassFileDescriptor> types = query("MATCH (t:Type) WHERE t.name='Pojo' and t.javaVersion starts with 'Java ' RETURN t").getColumn("t");
        assertThat(types).hasSize(1);
        TypeClassFileDescriptor pojo = types.get(0);
        assertThat(pojo).is(typeDescriptor(Pojo.class));
        store.commitTransaction();
    }
}
