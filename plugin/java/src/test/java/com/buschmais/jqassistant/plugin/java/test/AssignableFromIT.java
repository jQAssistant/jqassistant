...
import com.buschmais.jqassistant.plugin.java.test.set.java.ClassType;
import com.buschmais.jqassistant.plugin.java.test.set.java.InterfaceType;
...
public class AssignableFromIT extends AbstractAnalysisIT {
    ...
    public void assignableFrom() throws IOException, AnalyzerException {
        ...
        assertThat(query("MATCH (types:TYPE)<-[:ASSIGNABLE_FROM]-(assignableType) RETURN assignableType").getColumn("assignableType"), allOf(hasItem(typeDescriptor(ClassType.class)), hasItem(typeDescriptor(InterfaceType.class)), hasItem(typeDescriptor(Object.class))));
    }
}
