...
| java.source.paths
| Comma separated list of scanned directory paths containing Java source files, e.g. "src,src-test,src-gen" (if used by CLI)
|
...
|====
| Name                    | Target label(s)              | Cardinality | Description
| HAS_SOURCE_FILE         | <<:File>>                    | 0..1        | References the Java source file
...
The jQAssistant *Java Plugin* plugin evaluates annotations provided by the *Java Annotation Library* which can be declared as dependency for the code of an application:
...
The annotation `com.buschmais.jqassistant.plugin.java.annotation.jQASuppress` can be used to suppress elements like classes, fields or methods from rule results.

In the following example any violation reported by constraint with id `my-rules:MyConstraint` will be suppressed for the annotated class:
...
If needed, any other column provided by the rule can be specified:

[source,java]
----
@jQASuppress(value = "my-rules:MyConstraint", column="TargetType")
public class ClassViolatingMyConstraint {
}
----

It is possible to set an expiry date and specify a reason for suppression. The date must be specified in the format "yyyy-MM-dd".
...
TIP: The `jQASuppress` annotation is repeatable.

=== @TechnicalDebt

The annotation `com.buschmais.jqassistant.plugin.java.annotation.TechnicalDebt` can be used to document technical debt on the level of Java types and/or their members.

TIP: The annotation is evaluated by the constraint `java:TechnicalDebt` to create reports.

The annotation requires at least a message describing the technical debt:

[source,java]
----
@TechnicalDebt(value = "This class must be migrated to ...")
public class ClassWithTechnicalDebt {
}
----

If needed, more optional information can be provided:
...
[source,java]
----
@TechnicalDebt(value = "This class must be migrated to ...", priority = HIGH, issue = "259", seeAlso = { RelatedClass.class })
public class ClassWithTechnicalDebt {
}
----

TIP: The `TechnicalDebt` annotation is repeatable.
