...
For using the jQAssistant *Java Plugin* it must be declared as compile-time dependency for your project, e.g. in case of Maven:
...
The java plugin evaluates annotations provided by the *Java Annotation Library*, which must be declared separately.
...
The annotation `com.buschmais.jqassistant.plugin.java.annotation.TechnicalDebt` may be used for marking as technical debt at types and members.
