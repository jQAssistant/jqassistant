...
Imports information from Maven POMs (e.g. `pom.xml`) files including Maven 4.1.0 POM format.

[NOTE]
====
When running under Maven 4, additional properties introduced in Maven 4.1.0 are detected via reflection and stored in the graph.
These include `root` on `:Maven:Pom`, `condition` on `:Maven:ProfileActivation`, `priority` on `:Maven:PluginExecution`, and `:Maven:Source` nodes.
When running under Maven 3, these properties are gracefully skipped.
====
