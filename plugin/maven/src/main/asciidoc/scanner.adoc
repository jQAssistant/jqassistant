...
| maven.dependencies.scan     | If set to `true` the declared dependencies will be scanned.                    | `false`
| maven.dependencies.includes | A comma separated list of artifact patterns to include in the dependency scan  | include all artifacts
| maven.dependencies.excludes | A comma separated list of artifact patterns to exclude from the dependeny scan | exclude no artifacts
|====
...
| HAS_SOURCE_DIRECTORY       | <<:File:Directory>>     | 0..n        | References the source directories of the project
| HAS_TEST_ SOURCE_DIRECTORY | <<:File:Directory>>     | 0..n        | References the test source directories of the project
...
| version    | The version
| root       | `true` if this is the root project (Maven 4.1.0, optional)
|====
...
| HAS_SOURCE          | <<:Maven:Source>>             | 0..n        | References a source definition (Maven 4.1.0, optional)
|====
...
| condition       | A condition expression for profile activation (Maven 4.1.0, optional)
|====
...
| priority  | The execution priority (Maven 4.1.0, optional)
|====
...
[[:Maven:Source]]
=== Nodes labeled with `:Maven:Source`
A source definition in a Maven 4.1.0 build.
Only available when running under Maven 4.

.Properties of `:Maven:Source`
[options="header"]
|====
| Name            | Description
| scope           | Context where source files are used, typically `main` or `test`
| lang            | Language of source files, typically `java` or `resources`
| module          | Name of the Java module being built (for multi-module projects)
| directory       | Directory where sources are stored (relative to POM)
| targetVersion   | Target platform version (e.g. compiler `--release` option for Java)
| targetPath      | Explicit target path, overriding the default value
| stringFiltering | Whether to filter tokens with parameterized values
| enabled         | Whether this source definition is included in the build
|====
