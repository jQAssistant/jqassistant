package com.buschmais.jqassistant.plugin.maven.test.rule;
...
class MavenIT extends AbstractPluginIT {
    ...
    void hierarchicalParentModuleRelation() throws Exception {
        ...
        assertThat(validateConstraint("maven:HierarchicalParentModuleRelation").getStatus()).isEqualTo(FAILURE);
        store.beginTransaction();
        List<Result<Constraint>> constraintViolations = new ArrayList<>(reportPlugin.getConstraintResults().values());
        assertThat(constraintViolations.size()).isEqualTo(1);
        Result<Constraint> result = constraintViolations.get(0);
        assertThat(result, result(constraint("maven:HierarchicalParentModuleRelation")));
